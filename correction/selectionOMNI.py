...
if len(qualified_params) == 0:
    ...
    # 保存结果
    qualified_df.to_csv(f"/home/zpw/Files/forbushDecrease/FDs/{EVENT_DIR}/qualified_omni_params.csv", index=False)
    print(f"\n[✓] 筛选结果已保存: /home/zpw/Files/forbushDecrease/FDs/{EVENT_DIR}/qualified_omni_params.csv")

    # ---------- 可视化：合格参数的热图 ----------
    print("\n生成合格参数热图...")
    qualified_param_names = qualified_df['parameter'].tolist()
    fig, ax = plt.subplots(figsize=(15, max(4, len(qualified_param_names)*0.3)))
    ...
    ax.set_title(
        f'Qualified OMNI parameters (>={MIN_RATIO:.0%} rigidity bins with $|\\rho|$ > {RHO_THRESHOLD}, p < {P_THRESHOLD})',
        fontsize=14, pad=20
    )
    ...
    plt.tight_layout()
    plt.savefig(f"/home/zpw/Files/forbushDecrease/FDs/{EVENT_DIR}/qualified_params_heatmap.png", dpi=300)
    plt.close(fig)
    print(f"[✓] 热图已保存: /home/zpw/Files/forbushDecrease/FDs/{EVENT_DIR}/qualified_params_heatmap.png")
