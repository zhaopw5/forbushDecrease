...
for rig in SELECTED_RIGS:
    ...
    gr.SetMarkerColor(ROOT.kBlack)  # 改为黑色
    gr.SetLineColor(ROOT.kBlack)    # 改为黑色
    ...
    # 拟合函数表达式
    latex.DrawLatex(0.5, 0.85-0.5, "Flux(t) = baseline - A#times exp[-#xi #times (#frac{#tau}{t-#Delta} + #frac{t-#Delta}{#tau} - 2)]")
    ...
    latex.DrawLatex(0.5, 0.78-0.48, f"baseline = {baseline:.3e} #pm {fred_fit.GetParError(0):.2e}")
    latex.DrawLatex(0.5, 0.74-0.48, f"#Delta = {Delta:.2f} #pm {fred_fit.GetParError(2):.2f} h")
    latex.DrawLatex(0.5, 0.66-0.45, f"#tau = {tau:.2f} #pm {fred_fit.GetParError(3):.2f} h")
    latex.DrawLatex(0.5, 0.62-0.45, f"#xi = {xi:.2f} #pm {fred_fit.GetParError(4):.2f}")
    latex.DrawLatex(0.5, 0.58-0.45, f"#chi^{{2}}/NDF = {chi2:.2f}/{ndf} = {chi2/ndf if ndf > 0 else 0:.2f}")
...
for idx, time_idx in enumerate(time_indices):
    ...
    latex.SetTextSize(0.035)
    latex.SetTextColor(ROOT.kBlue)
    latex.DrawLatex(0.12, 0.85-0.45, "Flux(R) = C ( #frac{R}{R_{0}} )^{#gamma_{1}} [ 1 + ( #frac{R}{R_{0}} )^{ #frac{#gamma_{2}}{s} } ]^{-s}")
    ...
    latex.DrawLatex(0.12, 0.78-0.45, f"C = {C:.3e} #pm {sbpl_fit.GetParError(0):.2e}")
    latex.DrawLatex(0.12, 0.74-0.45, f"#gamma_{{1}} = {gamma1:.3f} #pm {sbpl_fit.GetParError(1):.3f}")
    latex.DrawLatex(0.12, 0.70-0.45, f"R_{{0}} = {R0:.3f} #pm {sbpl_fit.GetParError(2):.3f} GV")
    latex.DrawLatex(0.12, 0.66-0.45, f"#gamma_{{2}} = {gamma2:.3f} #pm {sbpl_fit.GetParError(3):.3f}")
    latex.DrawLatex(0.12, 0.62-0.45, f"s = {s:.3f} #pm {sbpl_fit.GetParError(4):.3f}")
    latex.DrawLatex(0.12, 0.58-0.45, f"#chi^{{2}}/NDF = {chi2:.2f}/{ndf} = {chi2/ndf if ndf > 0 else 0:.2f}")
