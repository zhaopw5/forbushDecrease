...
for rig in SELECTED_RIGS:
    ...
    ndf = fred_fit.GetNDF()
    latex.DrawLatex(0.5, 0.30, f"baseline = {baseline:.3e} #pm {fred_fit.GetParError(0):.2e}")
    latex.DrawLatex(0.5, 0.26, f"A = {A:.3e} #pm {fred_fit.GetParError(1):.2e}")
    latex.DrawLatex(0.5, 0.22, f"#Delta = {Delta:.2f} #pm {fred_fit.GetParError(2):.2f} h")
    latex.DrawLatex(0.5, 0.18, f"#tau = {tau:.2f} #pm {fred_fit.GetParError(3):.2f} h")
    latex.DrawLatex(0.5, 0.14, f"#xi = {xi:.2f} #pm {fred_fit.GetParError(4):.2f}")
    latex.DrawLatex(0.5, 0.10, f"#chi^{{2}}/NDF = {chi2:.2f}/{ndf} = {chi2/ndf if ndf > 0 else 0:.2f}")
...
for idx, time_idx in enumerate(time_indices):
    ...
    latex.SetTextColor(ROOT.kBlue)
    latex.DrawLatex(0.12, 0.4, "Flux(R) = C ( #frac{R}{R_{0}} )^{#gamma_{1}} [ 1 + ( #frac{R}{R_{0}} )^{ #frac{#gamma_{2}}{s} } ]^{-s}")
    # 添加拟合参数值
    ...
    latex.DrawLatex(0.12, 0.33, f"C = {C:.3e} #pm {sbpl_fit.GetParError(0):.2e}")
    latex.DrawLatex(0.12, 0.29, f"#gamma_{{1}} = {gamma1:.3f} #pm {sbpl_fit.GetParError(1):.3f}")
    latex.DrawLatex(0.12, 0.25, f"R_{{0}} = {R0:.3f} #pm {sbpl_fit.GetParError(2):.3f} GV")
    latex.DrawLatex(0.12, 0.21, f"#gamma_{{2}} = {gamma2:.3f} #pm {sbpl_fit.GetParError(3):.3f}")
    latex.DrawLatex(0.12, 0.17, f"s = {s:.3f} #pm {sbpl_fit.GetParError(4):.3f}")
    latex.DrawLatex(0.12, 0.13, f"#chi^{{2}}/NDF = {chi2:.2f}/{ndf} = {chi2/ndf if ndf > 0 else 0:.2f}")
