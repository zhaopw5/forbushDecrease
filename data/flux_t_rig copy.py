...
for ts, row in ams.iterrows():
    # 取该时刻各刚度通量
    flux_series = row.reindex(flux_cols)
    if flux_series.isnull().all():
        continue
    flux = flux_series.values
    # 误差列若存在则使用，否则置零
    if all(c in ams.columns for c in err_cols):
        flux_err = row[err_cols].values
    else:
        flux_err = np.zeros_like(flux)
    # 绘制该时刻的能谱曲线（降低透明度，避免遮挡）
    plt.errorbar(
        rigidity_centers, flux,
        xerr=rigidity_errs, yerr=flux_err,
        fmt="-", lw=0.8, markersize=0, capsize=0, alpha=0.4,
        # 可按需打开图例，但曲线较多时不建议：
...
plt.ylabel("Flux")
plt.title("Proton Flux Spectra (Original Time Resolution)")
# 不显示图例，避免过多条目
